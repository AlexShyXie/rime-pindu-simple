...
def process_xiaohe_file(input_file, output_file):
    ...
    with open(input_file, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            # 记录偏旁到按键的映射
            if first_radical and third_letter:
                mapping_dict[first_radical] = third_letter
    ...
    # 生成映射文件
    mapping_file = 'radical_mapping.txt'
    with open(mapping_file, 'w', encoding='utf-8') as f:
        for radical, key in sorted(mapping_dict.items()):
            f.write(f"{radical}\t{key}\n")
    
    print(f"映射关系已保存到：{mapping_file}")
    print(f"共找到 {len(mapping_dict)} 个偏旁映射")
