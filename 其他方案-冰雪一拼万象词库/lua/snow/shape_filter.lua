...
-- === 辅助函数：加载按键提示映射 ===
---@param path string
function thistable_from_tsv(path)
  ...
  for line in file:lines() do
    ---@type string, string
    local character, content, anjian = line:match("([^\t]+)\t([^\t]+)\t([^\t]+)")
    if not content or not character then
      goto continue
    end
    result[character] = anjian
    ::continue::
  end
  file:close()
  return result
...
function filter.init(env)
  ...
  env.keyhint_map  = thistable_from_tsv(dir .. shape_mapping)
end
...
function filter.handle_candidate(text, shape_input, env)
  ...
  elseif id == "snow_yipin" then -- 冰雪一拼
    ...
    local key_hint = encode(element, env.keyhint_map)
    local comment = (key_hint .. " " .. element):gsub("rj", "'")
